...
class Agent(object):
    ...
    def __init__(self, agent_number, params={}):
        ...
        self.outlier_detection = 1.2 ## TODO: for part 1,this should be set to 1, for part 2 this should be set to 1.2
    ...
    #         print(" ===== zero_punishment_reaction_level_1 called ===== ")
    ...
    def zero_punishment_reaction_level_2(self):
        if self.opponent_last_alpha == 0 or (self.opponent_alpha_list[-2] - self.opponent_last_alpha) / self.opponent_alpha_list[-2] > 0.5:
            if np.random.rand() > 0.5:
                ...
                self.price_returned = True
                self.price_stored_for_current_round = [self.current_customer_valuation_item0 * scaled_random_number, self.current_customer_valuation_item1 * scaled_random_number]
                print(" ===== zero_punishment_reaction strategy 1 called ===== ")
            else:
                min_value = 0.8
                max_value = 0.9
                scaled_random_number = min_value + (max_value - min_value) * np.random.rand()
                self.price_returned = True
                self.price_stored_for_current_round = [self.current_customer_valuation_item0 * scaled_random_number, self.current_customer_valuation_item1 * scaled_random_number]
                print(" ===== zero_punishment_reaction strategy 2 called ===== ")
    ...
    def action(self, obs):
        ...
        if self.project_part == 2:
            ########## Getting our optimal prices for current user covariates ##########
            current_optimal_predictions = self.generate_optimal_prices(new_buyer_covariates)
            self.current_customer_valuation_item0 = current_optimal_predictions[0]
            self.current_customer_valuation_item1 = current_optimal_predictions[1]
            ######## END: Getting our optimal prices for current user covariates #######

            ########## setting alpha ##########
            ## First Stage: return ramdon alpha for ?? rounds
            if self.this_agent_number < self.random_rounds :
            ...
            if self.this_agent_number >= self.random_rounds:
                ...
                self.zero_punishment_reaction_level_2()
                self.punish_opponent()

                if self.price_returned:
                    return np.array(self.price_stored_for_current_round)
                else:
                    did_customer_buy_from_me = last_sale[1] == self.this_agent_number
            ...
            optimal_price = np.array(current_optimal_predictions) * (0.7 + (0.75 - 0.7) * np.random.rand())
            return optimal_price * self.alpha
        ...
        if self.project_part == 2:
            self.last_customer_valuation = current_optimal_predictions
        ########################################################################